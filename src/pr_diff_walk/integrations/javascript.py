...
def _js_config() -> LanguageConfig:
    return LanguageConfig(
        ...
        entity_kinds={"class", "function", "method", "variable", "module"},
    )


class JavascriptIntegration(LanguageIntegration):
    def __init__(self):
        super().__init__(_js_config())
    ...
    def parse_imports(
        ...
        for i, line in enumerate(lines, start=1):
            ...
            if src_file:
                ...
                edges.append(ImportEdge(file_path, src_file, "default", alias, i, "js"))
        ...
        for i, line in enumerate(lines, start=1):
            ...
            if m:
                ...
                if m_ns:
                    edges.append(ImportEdge(file_path, src_file, None, m_ns.group(1), i, "js"))
                ...
                if m_named:
                    for part in [p.strip() for p in m_named.group(1).split(",")]:
                        ...
                        edges.append(ImportEdge(file_path, src_file, orig, alias, i, "js"))
        return edges
    ...
    def parse_entities(self, file_path: str, lines: List[str]) -> List[EntityDef]:
        ...
        for i, line in enumerate(lines, start=1):
            s = line.strip()
            if m := re.match(r"^(?:export\s+)?class\s+([A-Za-z_]\w*)\b", s):
